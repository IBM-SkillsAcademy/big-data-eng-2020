...
#clear log files
echo "" > /workloadScripts/Adel/ex4p1_time
echo "" > /workloadScripts/Adel/ex4p3_time
echo "" > /workloadScripts/Adel/ex5_time

for (( n=$1; n<=$2; n++ ))
do
    current_student=student`echo $n | awk '{ printf "%04i\n", $0 }'`
    /bin/su -c "/workloadScripts/Adel/bde_ex3.sh $current_student" - $current_student &
done
wait
...
do
    current_student=student`echo $n | awk '{ printf "%04i\n", $0 }'`
    /bin/su -c "/workloadScripts/Adel/bde_ex4.sh $current_student" - $current_student &
    #sleep 2
done
wait
...
do
    current_student=student`echo $n | awk '{ printf "%04i\n", $0 }'`
    /bin/su -c "/workloadScripts/Adel/bde_ex5.sh $current_student" - $current_student &
    #sleep 2
done
wait
